...
replace_base_assets() {
    ...
    local -r release_json="${MICROSHIFT_ROOT}/assets/release/release-${ARCH}.json"

    # Replace MicroShift images with OKD upstream images
    for cur_image in $(jq -e -r  '.images | keys []' "${release_json}") ; do
        ...
        jq --arg a "${cur_image}" --arg b "${new_image}"  '.images[$a] = $b' "${release_json}" >"${temp_json}"
        mv "${temp_json}" "${MICROSHIFT_ROOT}/assets/release/release-${ARCH}.json"
    done

    if [ "${ARCH}" = "aarch64" ]; then
        local router_image
        router_image=$(jq -e -r '.images["haproxy-router"]' "${release_json}")
        echo "[${ARCH}] Reusing 'haproxy-router' for 'haproxy-router-haproxy32'"
        jq --arg img "${router_image}" '.images["haproxy-router-haproxy32"] = $img' "${release_json}" >"${temp_json}"
